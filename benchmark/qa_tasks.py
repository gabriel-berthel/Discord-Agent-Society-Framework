...
async def run_a1(client: PromptClient, prober: Prober, personality):
    ...
    responses = [await client.prompt(question['question'], 100, 'ADMIN', 1, f'outputs/a1_{client.name}_{client.agent.archetype}.txt') for question in questions]
...
async def run_a2(client: PromptClient, prober: Prober, dialogues):
    ...
    print(f'Probing agent: {client.name}')
    responses = [await client.prompt(question['question'], 100, 'ADMIN', 1, f'outputs/a2_{client.name}_{client.agent.archetype}.txt') for question in questions]
...
async def run_a3(client: PromptClient, prober: Prober, reflections):
    ...
    print(f'Probing agent: {client.name}')
    responses = [await client.prompt(question['question'], 100, 'ADMIN', 1, f'outputs/a3_{client.name}_{client.agent.archetype}.txt') for question in questions]
